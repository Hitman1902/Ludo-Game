...
const gameSlice = createSlice({
  ...
  reducers: {
    ...
    updatePlayerPieceValue: (state, action) => {
      ...
      const playerKey = playerNo as keyof typeof state;
      const playerPieces: any[] = state[playerKey];
      const piece = playerPieces.find(p => p.id === pieceId);
      state.pileSelectionPlayer = -1;

      if (piece) {
        console.log(
          `[Reducer] Updating piece ${pieceId}: pos=${pos}, travelCount=${travelCount}`,
        );
        piece.pos = pos;
        piece.travelCount = travelCount;
        const currentPositionIdx = state.currentPosition.findIndex(
          (p: any) => p.id === pieceId,
        );
        ...
        if (pos === 0) {
          ...
          if (currentPositionIdx !== -1) {
            state.currentPosition[currentPositionIdx] = {
              id: pieceId,
              pos,
              travelCount,
            };
