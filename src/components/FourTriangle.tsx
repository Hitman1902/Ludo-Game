...
const PlayerPieces: React.FC<PlayerPiecesProps> = memo(
  ({player, style, pieceColor, translate}) => {
    return (
      <View style={[styles.mainContainer, style]}>
        {player.map((piece, idx) => {
          // Use the correct transform key
...
const FourTriangle: React.FC<FourTriangleProps> = ({
  player1,
  player2,
  player3,
  player4,
  ...
  const renderPlayerPieces = useCallback(
    (data: PlayerPiecesItem, index: number) => {
      return (
        <PlayerPieces
          key={index}
