...
const Cell: React.FC<CellProps> = ({id, color}) => {
  ...
  const peicesAtPosition: any[] = useMemo(
  ...
  return (
    <View
      ...
      {peicesAtPosition.map((piece, index) => {
        // DEBUG LOGGING: Track rendering of pieces on the board
        console.log(`[Cell] Rendering piece ${piece.id} at position ${id}`);
        const playerNo =
          piece.id[0] === 'A'
            ? 1
            : piece.id[0] === 'B'
